...
function cabtek_preprocess_html(&$vars) {
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = cabtek_id_safe('page-'. $path);
    $vars['classes_array'][] = cabtek_id_safe('section-'. $section);
...
function cabtek_preprocess_page(&$vars, $hook) {
...
function cabtek_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from cabtek_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
//function cabtek_preprocess_block(&$vars, $hook) {
//  // Add a striping class.
 ...
 */
function cabtek_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('cabtek_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('cabtek_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('cabtek_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('cabtek_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('cabtek_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function cabtek_id_safe($string) {
 ...
 *
 */
function cabtek_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = cabtek_id_safe($element['#title']);
 ...
 */
function cabtek_preprocess_menu_local_task(&$variables) {
 ...
 */
function cabtek_menu_local_tasks(&$variables) {
  $output = '';
...
function cabtek_form_alter(&$form, $form_state, $form_id) {
  ...
  //if ($form_id === 'commerce_cart_add_to_cart_form_1810') { 
			$form['quantity']['#description'] = '<b>*All prices shown are GST inclusive and updated based on your selections.</b>';
    }
}